  ...
  shiny::moduleServer(
    ...
    function(input, output, session) {
      ...
      output$samlerapport <- shiny::renderUI({
        rapbase::renderRmd(
          system.file("samlerapport.Rmd", package = "korsband"),
      ...
      output$downloadSamlerapport <- shiny::downloadHandler(
        filename = function() {
          basename(tempfile(pattern = "korsbandSamlerapport",
                            fileext = paste0(".", input$formatS)))
        },
        content = function(file) {
          srcFile <-
            normalizePath(system.file("samlerapport.Rmd", package = "korsband"))
